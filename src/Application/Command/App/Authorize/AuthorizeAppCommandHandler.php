...
final class AuthorizeAppCommandHandler
{
     ...
     */
    public function __invoke(AuthorizeAppCommand $command): ErrorResponse|AppResponse
    {
        ...
        if (null !== $app && $command->getAppId() === AppId::AliExpress->value) {
            try {
                $updatedApp = $this->aliexpressAccessTokenManager->exchangeTemporaryTokenWithAccessToken(
                    $command->getTenantId(),
                    $command->getToken()
                );
